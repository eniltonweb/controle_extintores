...
if ($error_message) {
    ...
    if ($result->num_rows > 0) {
        ...
        <p><strong>Próxima Manutenção Nível 2:</strong> <?php echo (!empty($extintor['proxima_manutencao_n2']) && $extintor['proxima_manutencao_n2'] != '0000-00-00') ? htmlspecialchars($extintor['proxima_manutencao_n2']) : 'Não disponível'; ?></p>
        ...
        <?php if ($user_level == 'bombeiro' && $liberado_inspecao) : ?>
            <p><a href="inspecao_nivel1.php?codigo=<?php echo htmlspecialchars($codigo); ?>">Realizar Inspeção de Nível 1</a></p>
        <?php endif; ?>

        <?php if ($user_level == 'fornecedor' && $liberado_manutencao) : ?>
            <p><a href="manutencao_nivel2.php?codigo=<?php echo htmlspecialchars($codigo); ?>">Realizar Manutenção de Nível 2</a></p>
        <?php endif; ?>
        <?php endif; ?>
        <?php
    } else {
        echo "<p>Nenhum extintor encontrado com o código fornecido.</p>";
    }
} else {
    echo "<p>Erro ao executar a consulta: " . htmlspecialchars($conn->error) . "</p>";
}
