...
if ($result) {
    if ($result->num_rows > 0) {
        ...
        if ($user_level == 'bombeiro') {
            $sql_inspecao = "SELECT * FROM liberacao_inspecao WHERE codigo_extintor = '$codigo' AND liberado_para = 'bombeiro'";
            $result_inspecao = $conn->query($sql_inspecao);
        ...
        if ($user_level == 'fornecedor') {
            $sql_manutencao = "SELECT * FROM liberacao_manutencao WHERE codigo_extintor = '$codigo' AND liberado_para = 'fornecedor'";
